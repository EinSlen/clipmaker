...
export function AutomationPanel() {
  ...
  function changeGame(index: number, id: GameId) {
    ...
    updateChannel(index, (channel) => ({
      ...
      game: {
        ...
        musicProfile: id === "soft-body-slide" ? channel.game.musicProfile : undefined,
  ...
  async function syncGitHub() {
    ...
    try {
      ...
      const saveResponse = await fetch("/api/publisher/config", {
        method: "PUT",
        headers,
        body: JSON.stringify(config),
      });
      const saved = await saveResponse.json();
      if (!saveResponse.ok || !saved.ok) throw new Error(saved.error || "Sauvegarde impossible.");
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="panel p-5 sm:p-7" aria-labelledby="github-cloud-title">
                ...
                <div className="space-y-6 p-5 sm:p-6">
                  ...
                  <details className="subpanel p-4">
                    ...
                    <div className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                      {channel.game.id === "soft-body-slide" && <div className="space-y-2 text-xs text-ink-400 sm:col-span-2"><label className="block space-y-1.5"><span>Voix et ambiance</span><select className="field-control h-11" value={channel.game.musicProfile || "original"} onChange={(event) => updateChannel(index, (item) => ({ ...item, game: { ...item.game, musicProfile: event.target.value as PublisherChannelConfig["game"]["musicProfile"] } }))}><option value="edit-auto">Voix d’edit — mix aléatoire</option><option value="edit-sad">Voix d’edit — triste</option><option value="edit-revenge">Voix d’edit — revenge</option><option value="original">Ambiance originale — sans paroles</option><option value="auto">Chansons NCS — mix (option historique)</option><option value="revenge">Chansons NCS — puissant</option><option value="sad-english">Chansons NCS — mélancolique</option></select></label><p>Rotation quotidienne d’extraits parlés complets. Voix prioritaire, sans boucle. Bibliothèque vide = ce mode est bloqué.</p><a className="text-cyan-300 underline" href="https://einslen.github.io/clipmaker/#audio-library" target="_blank" rel="noreferrer">Gérer les voix dans le cloud ↗</a></div>}
